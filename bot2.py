...
def _provision_vps_sync(image_tag: str, ram_str: str, cpu_cores: int, disk_str: str, container_name: str, root_password: str):
    ...
    storage_opts = {}
    if disk_str.lower().endswith(('m', 'g')):
        storage_opts = {"size": disk_str.upper()}

    # Launch main OS container with soft memory reservation to allow virtual overcommit
    container_kwargs = {
        "image": image_tag,
        "name": container_name,
        ...
        "privileged": True,
        "mem_reservation": "256m"  # Allows oversold specs without crashing creation on low host RAM
    }

    if storage_opts:
        try:
            container_kwargs["storage_opt"] = storage_opts
            container = docker_client.containers.run(**container_kwargs)
        except docker.errors.APIError:
            # Fallback if host filesystem driver does not support storage quotas
            container_kwargs.pop("storage_opt", None)
            container = docker_client.containers.run(**container_kwargs)
    else:
        container = docker_client.containers.run(**container_kwargs)
    ...
    # Launch Tailscale container sharing the OS container network
    ...
    # Allow tailscaled to initialize
    time.sleep(3)

    # Force Tailscale to generate the authentication URL directly via exec
    auth_url = None
    for _ in range(10):
        try:
