...
def parse_size_to_bytes(size_str: str) -> int:
    ...
    if size_str in ["0", "0g", "0m", "unlimited"]:
        return 0
    match = re.match(r"^(\d+)([mg])$", size_str)
    if not match:
        raise ValueError("Invalid format. Use numbers followed by 'm' or 'g' (e.g., 512m, 2g, or 0 for unlimited).")
    num, unit = match.groups()
    num = int(num)
    bytes_val = num * 1024 * 1024 if unit == "m" else num * 1024 * 1024 * 1024
    if bytes_val < 256 * 1024 * 1024 and bytes_val != 0:
...
def _setup_tailscale_sync(container_id: str, auth_key: str, root_password: str) -> str:
    ...
    # Step 1: Install dependencies, fix SSH directory permissions, configure SSH & root login
    install_cmd = (
        ...
        "sed -i \"s/PasswordAuthentication.*/PasswordAuthentication yes/g\" /etc/ssh/sshd_config && "
        "mkdir -p /var/run/tailscale /var/lib/tailscale /var/run/sshd /run/sshd && "
        "chmod 0755 /var/run/sshd /run/sshd && "
        "chown root:root /var/run/sshd /run/sshd && "
        "ssh-keygen -A && "
...
def _build_and_run_sync(os_type: str, ram_bytes: int, cpu_cores: int, disk_bytes: int, container_name: str):
    ...
    # Cap requested CPU cores to host maximum available cores
    max_host_cpus = os.cpu_count() or 2
    if cpu_cores > max_host_cpus:
        cpu_cores = max_host_cpus
        
    nano_cpus = int(cpu_cores * 1_000_000_000) if cpu_cores > 0 else None

    # Map supported distribution OS names
    image_map = {
    ...
    if os_upper not in image_map:
        raise ValueError(f"Unsupported OS version: {os_type}. Supported: {', '.join(image_map.keys())}")
    ...
    if ram_bytes > 0:
        container_kwargs["mem_limit"] = ram_bytes
    if nano_cpus:
        container_kwargs["nano_cpus"] = nano_cpus
