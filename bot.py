...
def _setup_tailscale_sync(container_id: str, auth_key: str) -> str:
    ...
    # Start Tailscaled Daemon inside container
    container.exec_run("bash -c 'tailscaled --state=/var/lib/tailscale/tailscaled.state --socket=/var/run/tailscale/tailscaled.sock &'", detach=True)
    time.sleep(3)

    # Authenticate Tailscale using Auth Key
    up_cmd = f"tailscale up --authkey={auth_key} --hostname={container.name}"
