...
def _setup_tailscale_sync(container_id: str, root_password: str, os_key: str) -> str:
    ...
    # Configure APT endpoints and install prerequisites based on OS target
    if os_key in ["DEBIAN10", "DEBIAN11"]:
        repo_fix = (
    ...
    prep_cmd = (
        ...
        "apt-get install -y --allow-unauthenticated -qq curl ca-certificates gnupg iproute2 iptables openssh-server >/dev/null 2>&1 && "
    ...
    res_prep = container.exec_run(prep_cmd)
    if res_prep.exit_code != 0:
        raise RuntimeError(f"Base setup failed: {res_prep.output.decode('utf-8', errors='ignore')}")

    # Start SSH Server
    ssh_start = container.exec_run("/usr/sbin/sshd")
    if ssh_start.exit_code != 0:
        raise RuntimeError(f"SSH launch failed: {ssh_start.output.decode('utf-8', errors='ignore')}")

    # Install Tailscale via official script
    ts_install = container.exec_run("bash -c 'curl -fsSL https://tailscale.com/install.sh | sh >/dev/null 2>&1'")
    if ts_install.exit_code != 0:
        raise RuntimeError(f"Tailscale installation failed: {ts_install.output.decode('utf-8', errors='ignore')}")
    ...
    up_cmd = f"bash -c 'tailscale --socket=/var/run/tailscale/tailscaled.sock up --hostname={clean_hostname} --reset 2>&1'"
    ...
    raise RuntimeError(f"Failed to capture Tailscale login link. Output: {output.strip()}")
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        ...
        await status_msg.edit(content="⏳ **[2/3]** Configuring SSH & Generating Tailscale login link...")
        ...
        await status_msg.edit(content=f"⏳ **[3/3]** Delivering authentication details to {user.mention} via DM...")

        vps_id = container.id[:10]

        dm_embed = discord.Embed(
            title="🚀 Your Tailscale VPS is Online!",
            description="Open the Tailscale Auth Link below to bind this VPS to your Tailscale account.",
        ...
        dm_embed.add_field(name="🔑 Tailscale Login Link", value=f"[**Authenticate Tailscale Node**]({login_url})\n`{login_url}`", inline=False)
