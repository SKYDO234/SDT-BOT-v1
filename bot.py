...
def _provision_vps_sync(image_tag: str, ram_bytes: int, cpu_cores: int, disk_bytes: int, container_name: str, root_password: str):
    ...
    # Launch main OS container with OpenSSH Server
    ...
    # Launch Tailscale container linked directly to the main container's network namespace
    ...
    # Strictly capture ONLY user interactive authentication URLs
    while time.time() - start_time < 30:
        logs = ts_container.logs().decode("utf-8", errors="ignore")
        
        # Regex strictly isolated to user authentication endpoints
        matches = re.findall(r"https://login\.tailscale\.com/a/[a-zA-Z0-9]+", logs)
        if matches:
            auth_url = matches[-1] # Take the most recent auth URL generated
            break
    ...
    if not auth_url:
        ...
        ts_container.stop()
        ts_container.remove()
        raise RuntimeError("Failed to capture Tailscale Auth URL. Please verify internet routing on the host machine.")
