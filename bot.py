...
# ---------------------------------------------------------
def _setup_tailscale_sync(container_id: str, root_password: str) -> str:
    ...
    # Step 6: Trigger interactive Tailscale auth and extract login URL
    up_cmd = f"tailscale --socket=/var/run/tailscale/tailscaled.sock up --hostname={clean_hostname}"
    res_up = container.exec_run(up_cmd)
    output = res_up.output.decode("utf-8", errors="ignore")

    # Search for login URL in command output
    ...
    raise RuntimeError(f"Failed to capture Tailscale login URL. Output: {output}")

async def setup_tailscale_container(container, root_password: str) -> str:
    return await asyncio.to_thread(_setup_tailscale_sync, container.id, root_password)
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    try:
        ...
        await status_msg.edit(content="⏳ **[2/3]** Configuring SSH & Tailscale authentication link...")

        login_url = await asyncio.wait_for(
            setup_tailscale_container(container, root_password),
            timeout=120.0
        )

        await status_msg.edit(content=f"⏳ **[3/3]** Sending authentication link to {user.mention} via Direct Message...")

        vps_id = container.id[:10]

        dm_embed = discord.Embed(
            title="🚀 Your Tailscale VPS is Provisioned!",
            description="Complete setup by opening the login link below to register this instance to your Tailscale network.",
        ...
        dm_embed.add_field(name="🔑 Tailscale Auth Link", value=f"[**Click Here to Authenticate Tailscale**]({login_url})\n`{login_url}`", inline=False)
        ...
        try:
            ...
            container.stop()
            container.remove()
            await status_msg.edit(content=f"❌ **Deployment Aborted:** Could not send DM to {user.mention}. Please enable DMs and try again.")
            return
