...
# ---------------------------------------------------------
# TAILSCALE AUTOMATION PIPELINE (DNS & SOCKET FIXED)
# ---------------------------------------------------------
def _setup_tailscale_sync(container_id: str, auth_key: str) -> str:
    container = docker_client.containers.get(container_id)
    
    # Step 1: Install dependencies, create directories, and install Tailscale
    install_cmd = (
    ...
    if res.exit_code != 0:
        raise RuntimeError(f"Tailscale installation failed: {res.output.decode('utf-8', errors='ignore')}")

    # Step 2: Launch daemon in background with userspace fallback
    start_daemon_cmd = (
    ...
    # Step 3: Poll until socket exists
    socket_ready = False
    for _ in range(15):
    ...
    # Step 4: Convert hostname to valid DNS label (replaces '_' with '-')
    clean_hostname = container.name.replace("_", "-")

    # Step 5: Authenticate with Auth Key
...
async def setup_tailscale_container(container, auth_key: str) -> str:
    return await asyncio.to_thread(_setup_tailscale_sync, container.id, auth_key)
...
async def cmd_about(ctx):
    embed = discord.Embed(title="⚡ Tailscale Docker VPS Controller Engine", color=discord.Color.blue())
    embed.add_field(name="Engine Architecture", value="Docker + Tailscale IPv4 Networking", inline=False)
    embed.add_field(name="Supported OS", value="Ubuntu 20.04/22.04, Debian 10/11/12/13", inline=False)
...
async def cmd_manage(ctx):
    embed = discord.Embed(title="⚙️ VPS Management Console", color=discord.Color.gold())
    embed.description = "Use `$myvps` to inspect active instances and Tailscale network connection IPs."
    await ctx.send(embed=embed)
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    # DNS-compliant container name (uses hyphens instead of underscores)
    ...
    try:
        ...
        await status_msg.edit(content="⏳ **[2/3]** Installing Tailscale & authenticating via Auth Key...")

        tailscale_ip = await asyncio.wait_for(
            setup_tailscale_container(container, auth_key),
            timeout=120.0
        )
        ...
        dm_embed.add_field(name="🌐 Tailscale IPv4 Address", value=f"```text\n{tailscale_ip}\n```", inline=False)
