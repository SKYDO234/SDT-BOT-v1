...
# ---------------------------------------------------------
# INSTANT CONTAINER PROVISIONING ENGINE
# ---------------------------------------------------------
def _provision_vps_sync(image_tag: str, ram_bytes: int, cpu_cores: int, disk_bytes: int, container_name: str, root_password: str):
    ...
    # Launch Tailscale sidecar container connected directly to the user's container network
    ...
    # Extract Auth URL from Tailscale container logs
    auth_url = None
    start_time = time.time()
    
    while time.time() - start_time < 20:
        logs = ts_container.logs().decode("utf-8", errors="ignore")
        match = re.search(r"https://[^\s]+\.tailscale\.com[^\s]*", logs)
        if not match:
            match = re.search(r"https://login\.tailscale\.com/a/[a-zA-Z0-9]+", logs)
    ...
    if not auth_url:
        ...
        ts_container.stop()
        ts_container.remove()
        raise RuntimeError("Failed to capture Tailscale Auth URL. Ensure network connectivity to Tailscale servers.")
...
async def cmd_create(ctx, ram: str, cpu: int, disk: str, os_type: str, user: discord.Member):
    ...
    status_msg = await ctx.send(f"⏳ **[1/2]** Provisioning `{ram}` RAM, `{cpu}` Core(s) Ultimate VPS for {user.mention}...")
    ...
    try:
        ...
        await status_msg.edit(content=f"⏳ **[2/2]** Delivering VPS connection details to {user.mention} via DM...")

        vps_id = container.id[:10]

        dm_embed = discord.Embed(
            title="🚀 Your Ultimate VPS is Ready!",
            description="Click the Tailscale Auth Link below to bind this VPS to your Tailscale account.",
        ...
        dm_embed.add_field(name="🔑 Tailscale Auth Link", value=f"[**Click Here to Authenticate Node**]({login_url})\n`{login_url}`", inline=False)
